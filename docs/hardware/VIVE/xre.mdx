...
The Vive XR Elite is a standalone VR headset powered by the Qualcomm XR2, similar to its predecessor the Vive Focus 3.
Although by default having neither eye nor face tracking, the XRE has an add-on module that can be installed to add eye and face tracking capabilities.
...
The XR Elite setup for VRCFT is essentially the same as that for the [Vive Focus 3](./focus3.mdx). 
...
While users can choose to simply forgo using VRCFT, we would recommend *against* using the Vive Streamer built-in outputs and follow this guide to use VRCFT instead. 
We will be unable to provide support to users who experience issues with the VRCFT clone in Vive's streamer software. 

:::warning
The XR Elite eye and facial tracker currently has some issues with certain expressions, such as EyeWide. 
Future updates *may* improve the behavior of these bugged? tracked parameters. 
:::
...
For most users, the Vive Streamer will be more straightforward to set up. 

### Preliminary Setup

1. Install the eye and face tracking module to the headset. 
...
3. Install VIVE Console onto your computer. We need this for the latest version (**1.3.6.10+**) of [SRanipal](./sranipal.mdx#installing-via-vive-console) 
...
1. Install VIVE Streaming Hub or VIVE Business Streaming onto your computer. They are functionally identical, but the XRE's feature to launch PCVR games such as VRChat from it's library relies on using Vive Streaming Hub. 
2. Update the streaming app on the XR Elite by plugging the headset into the computer then clicking the Update button in the VIVE Streaming application for "Headset software version".
...
There are 2 modules that can be used with the Vive XR Elite, one for each possible PCVR streaming method. 
