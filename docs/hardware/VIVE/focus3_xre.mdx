...
:::info
As of August 2, 2024, you should opt into the BETA version of the Vive Streaming Hub. The live version at the time of writing has various issues related to eye/face tracking. 
<div style={{
    width: '80%',
    height: 'auto',
    margin: 'auto',
...
1. Install VIVE Business Streaming or VIVE Streaming Hub onto your computer. They are functionally identical. The typical cases are VBS for the Focus 3 and the Streaming Hub for the XR Elite. 
2. Update the streaming app to the latest version on the Focus 3 or XR Elite by plugging the headset into the computer then clicking the Update button in the VIVE Streaming application for "Headset software version".
...
<details>
  <summary>Vive Streamer with SRanipal Module</summary>

  SRanipal was the original ET/FT method that was available for the Focus 3 headset. 
  ...
  2. Disable the OSC output from the Vive Streamer
   - Navigate to `C:\ProgramData\HTC\ViveSoftware\ViveRR\RRServer` and open up the `serverSetting.setting` file in your favorite text editor.
   - Scroll to the bottom of the file and set the `VOF` key to "**false**".
  3. Install the **SRanipalTrackingModule** module from the VRCFaceTracking module repository.
     - This should open a UAC prompt asking for permission to start the SRanipal runtime (sr_runtime).
...
The Vive Streaming module is closed source. 


## Troubleshooting

<details>
  <summary>My avatar's lower lip is up too high/clipping when I have a neutral expression</summary>
  <TroubleShootTable
  cause="You're using the Vive Streamer's built in output">

  Follow the instructions on this page to use VRCFT instead. 
  The mouth clipping issue is only caused by the Vive Streamer Hub, so if you see it, either you did not setup VRCFT and your chosen module correctly, or you did not set up VRCFT at all. 
