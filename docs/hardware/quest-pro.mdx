...
There have been a non-insignificant number of users who have experienced the exact same failure mode
...
SteamLink is Valve's just-released PCVR steaming solution meant specifically for using Quest headsets with SteamVR, meaning setup is a breeze but the software may be prone to various bugs. 
...
:::warning
Oculus PTC version v60 will cause any Link-reliant VRCFT module to break, even though the module may initialize. This may be resolved in future v60 updates on the PTC channel or in the live release.
:::
...
1. [**MAKE SURE YOUR HEADSET HAS EYE AND FACE TRACKING ENABLED**](#enabling-eye-and-face-tracking-on-the-headset)
2. Once connected to SteamVR, open the SteamVR menu, open Settings, and enable "Advanced Settings" by moving the toggle to "Show" in the bottom left.
  <div style={{
    width: '75%',
    height: 'auto',
    margin: 'auto',
...
<details>
  ...
  You may also need to re-verify developer account settings if using Oculus Link/Airlink. 
  </TroubleShootTable>
</details>
