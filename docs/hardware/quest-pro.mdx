...
where eye and face tracking are nonfunctional only when attempting using eye and face tracking simultaneously. 
The issue seems to be caused by moisture which has condensed inside the headset, eventually corroding the
face tracking IR illuminators. 
[More on the issue and potential prevention and repair methods.](#quest-pro)
:::
