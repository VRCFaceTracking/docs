...
:::danger
The Quest Pro is largely susceptible to moisture damage. 
There have been a large number of users who have experienced the exact same failure mode
where eye and face tracking are nonfunctional when attempting using eye and face tracking simultaneously. 
The issue seems to be caused by moisture which has condensed inside the headset eventually corroding 
face tracking IR illuminators, resulting in "green crud" that prevents the illuminators from working correctly. 
[More on the issue and potential prevention and repair methods.]()
:::
