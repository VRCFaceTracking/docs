...
* If you are using a Vive Facial Tracker or other Vive hardware that relies on SRanipal, download the SRanipal module and add it to the VRCFaceTracking CustomLibs folder.

* If you are using some other hardware such as a Quest Pro, you will need to download the respective module for the headset, add it to the VRCFaceTracking CustomLibs folder, then run the program. 
You can find more instructions specific to your hardware on its documentation page in [Modules](../tutorial-modules/modules.mdx)
