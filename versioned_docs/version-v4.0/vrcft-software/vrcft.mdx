...
VRCFaceTracking has the SRanipal module built-in as default, but other modules for other headsets or hardware can be added. 

## Getting started with VRCFaceTracking

Download the latest version from the <CustomLink to="github" append="/tree/v4.0.0"/> repo!

* If you are using a Vive Facial Tracker or other hardware that relies on SRanipal, simply launch the downloaded VRCFaceTracking executable. 
It will prompt you to start SRanipal if it wasn't running already. 

* If you are using some other hardware such as a Varjo headset or HP Reverb Omnicept, you will need to download the respective module, add it to the VRCFaceTracking CustomLibs folder, then run the program. 
You can find more instructions specific to your hardware on its documentation page here or Github page. 
...
There are occasionally small things that are good to be aware of when using face tracking with VRCFT and VRChat.
